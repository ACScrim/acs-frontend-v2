  ...
  actions: {
    ...
    async createTournamentPhase(tournamentId: string, phaseData: TournamentPhase) {
      try {
        ...
        useToastStore().error(`Error creating tournament phase: ${error?.message || 'Unknown error'}`);
        throw error;
      }
    },
    async updateTournamentPhase(tournamentId: string, phaseId: string, phaseData: Partial<TournamentPhase>) {
      try {
        ...
        useToastStore().error(`Error updating tournament phase: ${error?.message || 'Unknown error'}`);
        throw error;
      }
    },
    async deleteTournamentPhase(tournamentId: string, phaseId: string) {
      try {
        ...
        useToastStore().error(`Error deleting tournament phase: ${error?.message || 'Unknown error'}`);
        throw error;
      }
    }
  }
